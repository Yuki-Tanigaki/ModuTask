...
def make_mock_robot(modules, robot_coord=(0.0, 0.0)):
    ...
    robot.install_module = MagicMock()
    return robot

class TestAssembly(unittest.TestCase):
    def test_successful_assembly(self):
        """ 部品がロボットと異なる位置にあり、インストールされるケース """
        ...
        robot.install_module.assert_called_once_with(module)
        self.assertEqual(task.completed_workload, 1.0)

    def test_module_already_installed(self):
        """ モジュールの座標がロボットと一致しておりインストール不要なケース """
        ...
        robot.install_module.assert_not_called()
